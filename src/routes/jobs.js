...
// GET /api/jobs/history — get paginated batch history
router.get('/history', async (req, res) => {
  try {
    ...
    // Fetch batches with aggregated counts
    const historyQuery = `
      SELECT 
        b.id, 
        b.total_urls, 
        ...
        COUNT(CASE WHEN j.status = 'active' OR j.status = 'pending' THEN 1 END) as pending
      FROM batches b
      ...
      LIMIT $1 OFFSET $2
    `;
    const result = await db.query(historyQuery, [limit, offset]);
...
router.post('/', upload.single('file'), async (req, res) => {
  try {
    ...
    await db.query(
      'INSERT INTO batches (id, total_urls) VALUES ($1, $2)',
      [batchId, urls.length]
    );

    // 2. Queue all URLs as jobs
    for (const url of urls) {
      await db.query(
        'INSERT INTO jobs (batch_id, url, status) VALUES ($1, $2, $3)',
        [batchId, url, 'pending']
      );
    }
    ...
    console.error('Submit jobs error:', err.message);
    res.status(500).json({ error: err.message });
  }
});

// GET /api/jobs/:batchId — get status of all jobs in a batch
router.get('/:batchId', async (req, res) => {
  try {
    const { batchId } = req.params;

    // Verify batch exists
    const batchRes = await db.query('SELECT * FROM batches WHERE id = $1', [batchId]);
    if (batchRes.rows.length === 0) {
      return res.status(404).json({ error: 'Batch not found' });
    }

    // Get jobs for the batch
    const jobsRes = await db.query(
      `SELECT id, url, status, metadata, error_message FROM jobs WHERE batch_id = $1 ORDER BY id ASC`,
      [batchId]
    );
    ...
    const jobs = jobsRes.rows.map((row) => ({
      ...
      url: row.url,
      status: row.status,
      result: row.metadata ? { metadata: row.metadata } : null,
    ...
    res.json({
      batchId,
      total: batchRes.rows[0].total_urls,
      completed,
      failed,
      pending,
      jobs,
    ...
    console.error('Get jobs details error:', err.message);
    res.status(500).json({ error: err.message });
  }
});

// GET /api/jobs/:batchId/download — download ZIP of results from S3
  ...
  try {
    ...
    if (!bucketName) {
      return res.status(500).json({
        error: 'AWS S3 bucket configuration is missing. Cannot download zip.',
      });
    }

    // List all files in S3 under scrapes/{batchId}/
    let continuationToken = undefined;
    const s3Objects = [];
    do {
      const listParams = {
        Bucket: bucketName,
        Prefix: `scrapes/${batchId}/`,
        ContinuationToken: continuationToken,
      };
      const listResponse = await s3.send(new ListObjectsV2Command(listParams));
      if (listResponse.Contents) {
        s3Objects.push(...listResponse.Contents);
      }
    ...
    if (s3Objects.length === 0) {
      return res.status(404).json({
        error: 'No files found in S3 for this batch. The scraper might still be running or all jobs failed.',
      });
    }

    res.attachment(`results_${batchId}.zip`);
    const archive = new ZipArchive({ zlib: { level: 6 } });
    archive.pipe(res);

    // Download files from S3 and stream directly to ZIP archiver
    for (const obj of s3Objects) {
      const relativePath = obj.Key.replace(`scrapes/${batchId}/`, '');
      try {
        ...
        console.error(`[ZIP] Failed to retrieve ${obj.Key} from S3:`, err.message);
      }
    }
...
// DELETE /api/jobs/:batchId — clean up batch results
